...
app.post("/api/register/participantes", (req, res) => {
    ...
    //Busqueda de código
    const sqlFind = "SELECT * FROM codigos WHERE codigo=?";
    db.query(sqlFind, codigo, (err, result) => {
        if(result.length > 0){
            //Validación de cantidad de equipos por código
            console.log('Existe');
            //Validación del nombre del equipo 
            const sqlNB = "SELECT * FROM equipos WHERE nombre_equipo=?";
            db.query(sqlNB, estu0.equipo, (err, result) => {
                if(result.length > 0){
                    res.send({
                    ...
                    const sqlCFind = "SELECT * FROM codigos WHERE equipos_disponibles <=> equipos_registrados AND codigo=?";
                    db.query(sqlCFind, codigo, async (err, result) => {
                        if(result.length > 0){
                            res.send({
                                message: "Límite de equipos excedido"
                            });
                            ...
                            const sqlU = "UPDATE codigos SET equipos_registrados = (equipos_registrados + 1) WHERE codigo=?";
                            db.query(sqlU, codigo, (err, result) => {
                                if(err){
                                    console.log(err);
                                }else{
                                    res.send({
                            ...
                            contentHtml = `
                                ...
                                <ul>
                                    ...
                                    <li>Facebook: <a href="https://www.facebook.com/gmcpanama1">GMC Panamá</a></li>
                                    <li>Linkedin: <a href="http://linkedin.com/company/panamagmc">GMC Panamá</a></li>
                                </ul>
                                <br/>
